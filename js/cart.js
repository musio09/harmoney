 ...
 *  ----------------------------------------------------------------------------
 *  • Does NOT touch js/menu.js. It reads name/price from the menu cards that
 *    menu.js renders (.menu-item / .item-name / .item-price) and re-attaches
 *    its "＋ Add" buttons every time menu.js re-renders the menu.
 *  • Injects its own HTML + CSS, so index.html only needs one <script> tag.
 *  • Cart is saved in localStorage and survives refreshes.
 *  • "Order" builds a WhatsApp message to the cafe phone (from #cafePhone).
 * ==========================================================================*/
(function () {
  'use strict';

  var STORAGE_KEY = 'harmony_cart_v1';
  var cart = load();          // { key: { name, nameAm, price, currency, qty } }
  var currency = 'ETB';
  ...
  function save() {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(cart)); } catch (_) { /* ignore */ }
  }
  function count() {
    var n = 0; Object.keys(cart).forEach(function (k) { n += cart[k].qty; }); return n;
  }
  function total() {
    var t = 0; Object.keys(cart).forEach(function (k) { t += cart[k].qty * cart[k].price; }); return t;
  }

  // Parse one rendered menu card into { key, name, nameAm, price, currency }
  function readItem(card) {
    ...
    var name = '';
    nameEl.childNodes.forEach(function (n) { if (n.nodeType === 3) name += n.textContent; });
    name = name.trim();
    ...
    return { key: name + '|' + price, name: name, nameAm: nameAm, price: price, currency: currency };
  }
  ...
  var CSS = [
    ...
    '.cart-overlay{position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:300;opacity:0;visibility:hidden;transition:opacity .25s,visibility .25s}',
    '.cart-overlay.open{opacity:1;visibility:visible}',
    '.cart-drawer{position:fixed;top:0;right:0;bottom:0;width:min(420px,100%);background:var(--cream,#fbfaf6);z-index:301;',
    '  display:flex;flex-direction:column;transform:translateX(100%);transition:transform .28s ease;box-shadow:-6px 0 30px rgba(0,0,0,.2);font-family:inherit}',
    '.cart-drawer.open{transform:translateX(0)}',
    '.cart-head{background:linear-gradient(135deg,var(--primary,#7b1025) 0%,#9a1a32 100%);color:#fff;padding:18px 20px;display:flex;align-items:center;justify-content:space-between}',
    ...
    '.cart-line{background:#fff;border:1px solid var(--border,#e6e0d4);border-radius:var(--radius,14px);padding:12px 14px;display:flex;align-items:center;gap:10px}',
    ...
    '.cart-qty button{width:28px;height:28px;border-radius:50%;border:1px solid var(--border,#e6e0d4);background:var(--soft,#f4f1e9);',
    '  font-size:16px;font-weight:800;cursor:pointer;font-family:inherit;color:var(--text,#1e1e1e)}',
    ...
    '.cart-line-sub{font-weight:800;color:var(--secondary,#087443);white-space:nowrap;font-size:14px;min-width:70px;text-align:right}',
    ...
    '.cart-btn{flex:1;padding:12px;border-radius:999px;border:0;font-weight:800;font-size:14px;cursor:pointer;font-family:inherit;text-align:center;text-decoration:none;display:block}',
    '.cart-btn.primary{background:var(--secondary,#087443);color:#fff}',
    '.cart-btn.primary:hover{background:var(--secondary-light,#0a9b58)}',
    '.cart-btn.ghost{background:var(--soft,#f4f1e9);color:var(--text,#1e1e1e);flex:0 0 auto;padding:12px 16px}',
    '.cart-btn[aria-disabled="true"]{opacity:.5;pointer-events:none}',
    '.cart-toast{position:fixed;left:50%;bottom:84px;transform:translateX(-50%) translateY(20px);background:#1e1e1e;color:#fff;',
    '  padding:10px 16px;border-radius:999px;font-size:13px;font-weight:600;z-index:400;opacity:0;transition:all .25s;pointer-events:none;white-space:nowrap}',
  ...
  var HTML =
    ...
    '<aside class="cart-drawer" id="cartDrawer" aria-label="Your order" role="dialog">' +
      '<div class="cart-head"><h2>🛒 Your Order</h2>' +
        '<button class="cart-close" id="cartClose" type="button" aria-label="Close">&times;</button></div>' +
      '<div class="cart-body" id="cartBody"></div>' +
      '<div class="cart-foot">' +
        '<div class="cart-total"><span>Total</span><span id="cartTotal">0 ETB</span></div>' +
        '<div class="cart-actions">' +
          '<button class="cart-btn ghost" id="cartClear" type="button">Clear</button>' +
          '<a class="cart-btn primary" id="cartOrder" href="#" target="_blank" rel="noopener">Order via WhatsApp</a>' +
        '</div>' +
      '</div>' +
    '</aside>' +
    '<div class="cart-toast" id="cartToast"></div>';

  var fab, fabCount, overlay, drawer, body, totalEl, orderEl, toastEl, toastTimer;
  ...
  function mount() {
    ...
    orderEl = document.getElementById('cartOrder');
    ...
    document.getElementById('cartClear').addEventListener('click', function () {
      if (!count() || confirm('Clear your cart?')) { cart = {}; save(); render(); }
    });
    document.addEventListener('keydown', function (e) { if (e.key === 'Escape') close(); });

    // qty controls inside the drawer (event delegation)
    body.addEventListener('click', function (e) {
      ...
      if (btn.dataset.act === 'del' || line.qty <= 0) delete cart[key];
      save(); render();
    });
  }
  ...
  function decorateNow(root) {
    root.querySelectorAll('.menu-item').forEach(function (card) {
      if (card.querySelector('.cart-add')) { syncButton(card); return; }
  ...
  function syncButton(card) {
    ...
    // only touch the DOM when something changed (avoids observer feedback loops)
    if (btn.textContent !== label) btn.textContent = label;
    if (btn.classList.contains('in-cart') !== !!line) btn.classList.toggle('in-cart', !!line);
  }
  ...
  function add(item) {
    ...
    var line = cart[item.key];
    if (line) line.qty += 1;
    else cart[item.key] = { name: item.name, nameAm: item.nameAm, price: item.price, currency: item.currency, qty: 1 };
    save();
    render();
  ...
  function render() {
    ...
    if (!keys.length) {
      body.innerHTML = '<div class="cart-empty">Your cart is empty.<br>Tap <b>+</b> on any menu item to add it.</div>';
    } else {
        ...
        return (
          '<div class="cart-line">' +
            '<div class="cart-line-info">' +
              '<div class="cart-line-name">' + esc(l.name) + (l.nameAm ? ' <small>' + esc(l.nameAm) + '</small>' : '') + '</div>' +
              '<div class="cart-line-price">' + esc(fmt(l.price)) + ' each</div>' +
            '</div>' +
            '<div class="cart-qty">' +
              '<button type="button" data-act="dec" data-key="' + esc(k) + '" aria-label="Decrease">−</button>' +
              '<span>' + l.qty + '</span>' +
              '<button type="button" data-act="inc" data-key="' + esc(k) + '" aria-label="Increase">+</button>' +
            '</div>' +
            '<div class="cart-line-sub">' + esc(fmt(l.qty * l.price)) + '</div>' +
          '</div>'
        );
      }).join('');
    }
    ...
    orderEl.setAttribute('aria-disabled', keys.length ? 'false' : 'true');
    orderEl.href = keys.length ? whatsappLink() : '#';

    // keep the "+ / ×2" state on the menu cards in sync
    var root = document.getElementById('menuRoot');
    if (root) root.querySelectorAll('.menu-item').forEach(syncButton);
  }

  function cafePhone() {
    var a = document.getElementById('cafePhone');
    var raw = (a && a.getAttribute('href')) || 'tel:+251911903172';
    return raw.replace(/^tel:/, '').replace(/[^\d]/g, '');
  }

  function whatsappLink() {
    var cafeName = (document.getElementById('cafeName') || {}).textContent || 'Harmony Cafe';
    var lines = ['Hello ' + cafeName.replace(/^[^\w]*/, '').trim() + '! I would like to order:', ''];
    Object.keys(cart).forEach(function (k) {
      var l = cart[k];
      lines.push('• ' + l.qty + ' × ' + l.name + ' — ' + fmt(l.qty * l.price));
    });
    lines.push('', 'Total: ' + fmt(total()));
    return 'https://wa.me/' + cafePhone() + '?text=' + encodeURIComponent(lines.join('\n'));
  }
  ...
  function init() {
    ...
    render();
    decorate();
    // menu.js replaces #menuRoot's innerHTML on every refresh → re-add buttons
    var root = document.getElementById('menuRoot');
    if (root && window.MutationObserver) {
      // menu.js swaps the whole section list, so watching direct children is enough
  ...
  window.HarmonyCart = {
    ...
    items: function () { return JSON.parse(JSON.stringify(cart)); },
    total: total, count: count,
    clear: function () { cart = {}; save(); render(); }
  };
})();
